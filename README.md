...
- `.github/workflows/ci.yml` + `.lighthouserc.json`: CI on every push/PR — internal link & asset check (lychee in offline mode with `--root-dir .`, `--index-files index.html` and `--include-fragments`, so dead `#anchors` are caught too) plus Lighthouse CI assertions (mobile, 3 runs: performance ≥ 0.7, accessibility / best-practices / SEO ≥ 0.9). `404.html` and the Google Search Console verification file are excluded from auditing (`404.html` is intentionally `noindex`; the verification file must stay a bare one-line document).
